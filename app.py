...
# Verify the file exists and variables are loaded
...
if not os.getenv('DB_HOST'):
    st.error("❌ Environment variables not loaded from .env file")
...
def get_db_connection():
    ...
    try:
        ...
        elif hasattr(st, 'secrets') and 'connections' in st.secrets:
            conn = psycopg2.connect(
...
def get_sqlalchemy_engine():
    ...
    try:
        ...
        if os.getenv("DB_HOST"):
            # Use URL.create() to avoid special character issues
            url = URL.create(
